...
window.addEventListener('DOMContentLoaded', (event) => {
  (function ($, window, document, undefined) {
    "use strict";
    var url = "https://socket.3i.com.vn:49104"; //Use when run on publish
    ...
    async function uploadTempFile(url) {
      ...
      var res = await fetch(
        `https://os.3i.com.vn/MobileLogin/CreateTempFileFromUrl?url=${url}`,
        requestOptions
      );
    ...
    async function removeTempFile(url) {
      ...
      var json = await fetch(
        `https://os.3i.com.vn/MobileLogin/DeleteFileTemp?filePath=${url}`,
        requestOptions
      );
      var rs = json.json();
    }
    ...
    function addNewAttachFile(file, id) {
      ...
      playButton.onclick = function () {
        ...
        if (file.type == "DRIVER") {
          ...
          fetch(
            `https://os.3i.com.vn/MobileLogin/GetItemEdmsFile?Id=${file.catFileId}`,
            requestOptions
          )
            .then((res) => res.json())
            .then((res) => {
              ...
              if (file.extension == "docx") {
                window.open("https://os.3i.com.vn/WordViewer#", "_blank");
              }
              if (file.extension == "xlsx") {
                window.open("https://os.3i.com.vn/ExcelViewer#", "_blank");
              }
              if (file.extension == "pdf") {
                window.open("https://os.3i.com.vn/PdfViewer#", "_blank");
              }
            })
    ...
    async function handleZoomMeeting(config = zoomConfig) {
      ...
      try {
        ...
        if (!roomIdMeeting) {
          ...
          const pin = $("#pin")[0].value;
          const roomName = `meeting_schedule_${pin?.trim() ?? ''}_${idMeeting}`;
          socket.emit('initSmartworkRoom', { id: roomName, userId: userID, displayName: displayName, role: roleMeeting });
          socket.emit('joinRoom', { room: `Smart Work meeting_schedule_${pin?.trim() ?? ''}_${idMeeting}`, userID: userID });
          socket.emit('reloadMembers', `Smart Work meeting_schedule_${pin?.trim() ?? ''}_${idMeeting}`);
          socket.emit('fetch-data-request', `Smart Work meeting_schedule_${pin?.trim() ?? ''}_${idMeeting}`);
    ...
    function addNewMeeting(name, id) {
      ...
      joinButton.onclick = function () {
        jQuery.ajax({
          type: "POST",
          url:
            "https://os.3i.com.vn/MobileLogin/GetZoomSdkMeetingSchedule?id=" +
            id,
          contentType: "application/json",
          dataType: "JSON",
          success: function (rs) {
    ...
    $(function () {
      ...
      $("#login-form").on("submit", function (e) {
        ...
        if ($("#username")[0].value != "" && $("#pass")[0].value != "") {
          ...
          //vuong
          fetch(
            "https://os.3i.com.vn/MobileLogin/LoginNoCheckOnlineNodeJs",
            requestOptions
          )
            .then((response) => response.json())
            .then(async (result) => {
      ...
      $("#logout").on("click", function () {
        ...
        fetch(
          `https://os.3i.com.vn/MobileLogin/LogOutNodeJs?userName=${$("#username")[0].value
          }`,
          requestOptions
        )
          .then((response) => response.json())
      ...
      function getListUserOnline() {
        ...
        fetch(
          "https://os.3i.com.vn/MobileLogin/GetListUserOnlineNotepad",
          requestOptions
        )
          .then((response) => response.json())
          .then((result) => {
      ...
      $("#join_meeting").on("click", function () {
        ...
        fetch(
          `https://os.3i.com.vn/MobileLogin/GetListMeetingSchedule?UserName=${$("#username")[0].value
          }&TimeDay=${formattedToday}&id=${id}`,
          requestOptions
        )
          .then((response) => response.json())
          .then((result) => {
          ...
          if (check) {
            ...
            reader.onloadend = async function () {
              ...
              var response = await fetch(
                "https://os.3i.com.vn/MobileLogin/InsertFile",
                requestOptions
              );
      ...
      async function pushVideo(videoBlob, id, type) {
        ...
        if (!result.Error && result.Object) {
          ...
          const file = {
            ...
            catFileId: result.ID,
            name: `${id}.${type}`,
            url: "https://os.3i.com.vn/" + videoUrl,
            blob: videoBlob,
          };
          ...
          cameraRecorded.src = "https://os.3i.com.vn/" + videoUrl;
          cameraRecorded.load();
          ...
          document.body.appendChild(a);
          a.style = "display: none";
          a.href = "https://os.3i.com.vn/" + videoUrl;
          a.download = file.name;
          // a.click();
          // window.URL.revokeObjectURL(audioUrl);
          document.body.removeChild(a);
      ...
      // $("#icon-cover2").click(function () {
      //   isChoosePort2 = !isChoosePort2;
      //   if (!isChoosePort2) {
      //     canvas.getObjects().forEach((obj) => {
      //       obj.portMark && canvas.remove(obj.portMark);
      //     });
      ...
      //   if (isChoosePort2 && target && target.name != "curve-point") {
      //     if (!objCover) {
      //       objCover = target;
      ...
      //         if (pointer.x === line.x1 && pointer.y === line.y1) {
      ...
      $("#icon-cover2").on("click", function () {
        ...
        $(document).ready(function() {
            ...
            let objects = []; // Array to store objects or points
            let selectedPoints = []; // Array to store selected points
        
            // Example: Function to add points (or objects) to the canvas
            function addPoint(x, y, radius = 5) {
                objects.push({ x, y, radius });
                drawPoints();
            }
        
            // Example: Function to draw points on the canvas
            function drawPoints() {
            ...
            // Example: Function to draw a curve between two selected points
            function drawCurve(point1, point2) {
                ctx.beginPath();
                ctx.moveTo(point1.x, point1.y);
                const cp1x = (point1.x + point2.x) / 2;
                const cp1y = point1.y;
                const cp2x = (point1.x + point2.x) / 2;
                const cp2y = point2.y;
            ...
            // Example: Function to handle mouse clicks on the canvas
            canvas.addEventListener('click', function(event) {
                const rect = canvas.getBoundingClientRect();
                const mouseX = event.clientX - rect.left;
                const mouseY = event.clientY - rect.top;
        
                // Check if clicked on any point (or object)
                objects.forEach(point => {
                    if (Math.sqrt((mouseX - point.x) ** 2 + (mouseY - point.y) ** 2) <= point.radius) {
                        selectedPoints.push(point);
                        if (selectedPoints.length === 2) {
                            drawCurve(selectedPoints[0], selectedPoints[1]);
            ...
            // Example: Adding some points to the canvas (you would replace this with your actual objects)
            addPoint(100, 100);
            addPoint(300, 200);
            addPoint(500, 150);
        });
...
function makeLine2(
  ...
  var line = new fabric.Path(`M ${startX} ${startY} L ${endX} ${endY}`, {
    //  M 65 0 L 73 6 M 65 0 L 62 6 z
    fill: "",
    stroke: "#000",
    // objectCaching: false,
